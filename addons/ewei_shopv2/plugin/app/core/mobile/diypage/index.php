...
class Index_EweiShopV2Page extends AppMobilePage 
{
	public function main() 
	{
		...
		$result['banner'] = pdo_fetchall('select * from '.tablename('ewei_shop_adsense').' where uniacid="'.$_W['uniacid'].'" and type=2');
		app_json($result);
	}
	...
	public function getInfo() 
	{
		...
		if (empty($dataurl)) 
		{
			app_json(array( 'goods' => array(), 'type' => 'stores' ));
		}
		...
		if (!(empty($dataurl))) 
		{
			if (strpos($dataurl, '/pages/') === false) 
			{
				...
				else if ($dataParams[0] == 'groups') 
				{
					...
					$goodsql = 'SELECT id,title,subtitle,thumb,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND status > 0 AND deleted = 0 AND uniacid =' . $_W['uniacid'] . ' limit 0,' . $goodsnum;
