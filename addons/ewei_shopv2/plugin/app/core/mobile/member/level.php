...
class Level_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
    {
        ...
        //用户的信息
        $member = pdo_get('ewei_shop_member',['uniacid'=>$uniacid,'openid'=>$openid],['id','openid','nickname','realname','is_open',"FROM_UNIXTIME(expire_time) as expire"]);
        //待领取的优惠券  两个
        ...
        $record = pdo_fetchall('select * from '.tablename('ewei_shop_level_record').'where (openid = :openid or user-id = :user_id) and uniacid = "'.$uniacid.'" order by id desc',[':openid'=>$member['openid'],':user_id'=>$member['id']]);
    ...
    public function record()
    {
        ...
        $total = pdo_fetchcolumn('select count(1) from '.tablename('ewei_shop_level_record').'where (openid = :openid or user_id = :user_id) and uniacid = "'.$uniacid.'" and  (createtime < "'.$year_month.'" or status > 0)',[':openid'=>$member['openid'],':user_id'=>$member['id']]);
        //查询记录以及分页
        $record = pdo_fetchall('select * from '.tablename('ewei_shop_level_record').' where (openid = :openid or user_id = :user_id) and uniacid = "'.$uniacid.'" and (createtime < "'.$year_month.'" or status > 0) order by id desc LIMIT '.$pindex.','.$pageSize,[':openid'=>$member['openid'],':user_id'=>$member['id']]);
        foreach ($record as $key=>$item) {
	...
	//查找用户信息
        ...
        //添加订单
        $order_id = m('game')->addorder($openid,$order_sn,$money,'','购买年卡id=5');
        //微信支付
        ...
        m('game')->addmemberlog($openid,$order_sn,$money,$level_id."购买年卡");
    ...
    public function get()
    {
        ...
        //判断支付金额  是否正确
        $price = m('game')->change_address($address_id,$openid,$uniacid);
        ...
        //查询领取记录里面的已领过的状态
        $log = pdo_fetchall('select * from '.tablename('ewei_shop_level_record').'where uniacid = "'.$uniacid.'" and (openid = :openid or user_id = :user_id) and level_id = "'.$level_id.'" and status > 0',[':openid'=>$openid,':user_id'=>$member['id']]);
        ...
        //添加订单
        $order_id = m('game')->addorder($openid,$order_sn,$money,$member,$address_id,"领取年卡".$record["month"]."权益",$goods);
    ...
    public function cancel()
    {
        ...
        if(pdo_exists('ewei_shop_order',['uniacid'=>$uniacid,'id'=>$order_id])){
    ...
    public function address_list()
    {
        ...
        $list = pdo_fetchall('select * from '.tablename('ewei_shop_member_address').'where uniacid="'.$uniacid.'" and (openid = :openid or user_id = :user_id) and deleted = 0 order by isdefault desc,id desc',[':openid'=>$openid,':user_id'=>$member['id']]);
        if(!$list){
            show_json(-1,"暂无地址，请去添加地址");
        }
        $data = m('game')->change_address($list[0]['id'],$openid,$uniacid);
    ...
    public function change()
    {
        ...
        $address_id = $_GPC['address_id'];
        $openid = $_GPC['openid'];
        $data = m('game')->change_address($address_id,$openid,$uniacid);
    ...
    public function level_alert(){
        ...
        $openid = $_GPC['openid'];
        $member = m('member')->getMember($openid);
        $notice = pdo_fetch('select * from '.tablename('notice').'where uniacid = :uniacid and (openid = :openid or user_id = :user_id) and status = 1 and no_id = :no_id',[':uniacid'=>$uniacid,':openid'=>$member['openid'],':user_id'=>$member['id'],':no_id'=>$no_id]);
        if($notice){
            show_json(0,"已查看");
        }else{
            pdo_insert('notice',['uniacid'=>$uniacid,'openid'=>$openid,'user_id'=>$member['id'],'status'=>1,'no_id'=>$no_id,'createtime'=>time()]);
            show_json(1);
        }
    }
    ...
    public function goods_list()
    {
        ...
        $record = pdo_fetch('select * from '.tablename('ewei_shop_level_record').'where (openid = :openid or user_id = :user_id) and month = :month and status = 1',[':openid'=>$openid,':user_id'=>$member['id'],':month'=>$month]);
