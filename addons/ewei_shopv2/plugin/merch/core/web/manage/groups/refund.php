					...
					else 
					{
						...
						else 
						{
							if( $refundstatus == 1 ) 
							{
								...
								if( 0 < $item["credit"] ) 
								{
									m("member")->setCredit($item["openid"], "credit1", $item["credit"], array( "0", $shopset["name"] . "购物返还抵扣卡路里 卡路里: " . $item["credit"] . " 抵扣金额: " . $item["creditmoney"] . " 订单号: " . $item["orderno"] ));
								}
								if( !empty($refundtype) ) 
								{
