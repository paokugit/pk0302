...
class PosterProcessor extends PluginProcessor 
{
	...
	private function responseSubscribe($obj) 
	{
		...
		if( empty($log) && $openid != $qr["openid"] ) 
		{
			...
			{
				m("member")->setCredit($openid, "credit1", $poster["subcredit"], array( 0, "扫码关注卡路里+" . $poster["subcredit"] ));
			}
			...
			{
				m("member")->setCredit($qr["openid"], "credit1", $poster["reccredit"], array( 0, "推荐扫码关注卡路里+" . $poster["reccredit"] ));
			}
		...
		if( $p ) 
		{
			...
			if( !empty($cset) && $member["isagent"] != 1 && $qrmember["isagent"] == 1 && $qrmember["status"] == 1 ) 
			{
				if( !empty($poster["bedown"]) && empty($member["agentid"]) && empty($member["fixagentid"]) ) 
				{
                    ...
                    //添加绑定日志
                    $add = ['openid'=>$member['openid'],'item'=>'system','value'=>'绑定上级:'.$member['openid'].'/'.$member['nickname'].'绑定上级id:'.$qrmember['id'].'-'.$qrmember['nickname'],'create_time'=>date('Y-m-d H:i:s',time())];
