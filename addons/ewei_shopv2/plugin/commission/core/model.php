...
// 					$first = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_member") . " where uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"] ));
		...
		public function checkOrderConfirm($orderid = "0") 
		{
			...
			if( $parent_is_agent && $become_child == 1 && empty($member["agentid"]) && $member["id"] != $parent["id"] && empty($member["fixagentid"]) ) 
			{
                ...
                //添加绑定日志
                $add = ['openid'=>$member['openid'],'item'=>'system','value'=>'绑定上级:'.$member['openid'].'/'.$member['nickname'].'绑定上级id:'.$parent['id'].'-'.$parent['nickname'],'create_time'=>date('Y-m-d H:i:s',time())];
		...
		public function sendMessage($openid = "", $data = array( ), $message_type = "") 
		{
			...
			if( $message_type == TM_COMMISSION_AGENT_NEW && empty($usernotice["commission_agent_new"]) ) 
			{
				...
				if( $tm["is_advanced"] ) 
				{
					...
					$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，您新增了一个" . $set["texts"]["down"] . $data["nickname"], "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => "新增下级通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您新增了一个下级" . $data["nickname"], "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
					$toopenid = $openid;
			...
			else 
			{
				if( $message_type == TM_COMMISSION_ORDER_PAY && empty($usernotice["commission_order_pay"]) ) 
				{
					if( $tm["is_advanced"] ) 
					{
						...
						$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，您的" . $set["texts"]["down"] . $data["nickname"] . "已付款", "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => $set["texts"]["down"] . "付款通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您的下级" . $data["nickname"] . "已经成功付款", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
						$toopenid = $openid;
						$datas[] = array( "name" => "昵称", "value" => $data["nickname"] );
						$datas[] = array( "name" => "下级昵称", "value" => $data["nickname"] );
				...
				else 
				{
					if( $message_type == TM_COMMISSION_ORDER_FINISH && empty($usernotice["commission_order_finish"]) ) 
					{
						if( $tm["is_advanced"] ) 
						{
							...
							$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，您的" . $set["texts"]["down"] . $data["nickname"] . "已确认收货", "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => $set["texts"]["down"] . "收货通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您的下级" . $data["nickname"] . "已经确认收货", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
							$toopenid = $openid;
								...
								else 
								{
									if( $message_type == TM_COMMISSION_UPGRADE && empty($usernotice["commission_upgrade"]) ) 
									{
										if( $tm["is_advanced"] ) 
										{
											...
											$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，恭喜您成为" . $data["newlevel"]["levelname"] . $set["texts"]["agent"], "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => $set["texts"]["agent"] . "等级升级通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您的" . $set["texts"]["agent"] . "等级从" . $data["oldlevel"]["levelname"] . "升级为" . $data["oldlevel"]["levelname"] . "特此通知!", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
											$toopenid = $openid;
											$datas[] = array( "name" => "昵称", "value" => $member["nickname"] );
											$datas[] = array( "name" => "时间", "value" => $time );
									...
									else 
									{
										if( $message_type == TM_COMMISSION_BECOME && empty($usernotice["commission_become"]) ) 
										{
											if( $tm["is_advanced"] ) 
											{
												...
												$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，恭喜您" . $set["texts"]["become"], "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => "成为" . $set["texts"]["agent"] . "通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "成为" . $set["texts"]["agent"] . "通知", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
												$toopenid = $openid;
												$datas[] = array( "name" => "昵称", "value" => $data["nickname"] );
