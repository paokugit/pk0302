...
class Coupon_EweiShopV2ComModel extends ComModel 
{
	...
	public function payResult($logno) 
	{
		...
		if( empty($log["status"]) ) 
		{
			...
			{
				m("member")->setCredit($log["openid"], "credit1", 0 - $coupon["credit"], "购买优惠券扣除卡路里 " . $coupon["credit"]);
	...
	public function sendMessage($coupon, $send_total, $member, $account = NULL) 
	{
		...
		if( !empty($articles) ) 
		{
			...
			if( is_error($resp) ) 
			{
				...
				$msg = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "恭喜您获得优惠券", "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "优惠券通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => "获得" . $coupon["couponname"], "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => $desc, "color" => "#4b9528" ), "remark" => array( "value" => "点击查看详情", "color" => "#000000" ) );
				if( !empty($templateid) ) 
				{
	...
	public function sendBackMessage($openid, $coupon, $gives) 
	{
		...
		if( isset($gives["money"]) ) 
		{
			...
			$givestr .= (string) $gives["money"] . "元余额";
		}
		if( isset($gives["redpack"]) ) 
		{
			...
			$givestr .= (string) $gives["redpack"] . "元现金";
		}
		$content .= $givestr;
		$content .= "，请查看您的账户，谢谢!";
	...
	public function useRechargeCoupon($log) 
	{
		...
		if( !empty($backcredit) ) 
		{
			...
			if( 0 < $backcredit ) 
			{
				...
				m("member")->setCredit($data["openid"], "credit1", $backcredit, array( 0, "充值优惠券返卡路里" ));
			}
		}
	...
	public function backConsumeCoupon($orderid) 
	{
		...
		if( $parentid != 0 && $couponmerchid == 0 && $all_done == 1 ) 
		{
		...
		if( $canback ) 
		{
			...
			if( !empty($backcredit) ) 
			{
				...
				{
					$gives["credit"] = $backcredit;
					m("member")->setCredit($order["openid"], "credit1", $backcredit, array( 0, "充值优惠券返卡路里" ));
				}
			}
			if( !empty($backmoney) ) 
			{
