...
class Common_EweiShopV2Model 
{
	...
	//微信支付 fanbeibei
	public function fwechat_child_build($params, $wechat, $type = 0)
	{
	    global $_W;
	    load()->func("communication");
	    $package = array( );
	    ...
	    libxml_disable_entity_loader(true);
	    $prepayid = $xml->prepay_id;
	    $wOpt = array( "appId" => trim($wechat["appid"]), "timeStamp" => TIMESTAMP . "", "nonceStr" => random(32), "package" => "prepay_id=" . $prepayid, "signType" => "MD5" );
	    ksort($wOpt, SORT_STRING);
	    $string = "";
	    foreach( $wOpt as $key => $v )
	    {
