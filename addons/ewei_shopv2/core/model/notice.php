...
class Notice_EweiShopV2Model 
{
	...
	public function sendTeamMessage($orderid = "0", $delRefund = false) 
	{
		...
		$orderpricestr = " ¥" . $price . "元 (包含运费: ¥" . $order["freight"] . "元，卡路里抵扣: ¥" . $order["creditmoney"] . "元)";
		...
		if( $delRefund == true ) 
		{
			...
			$msg = array( "first" => array( "value" => "您的订单已经完成退款！", "color" => "#000000" ), "keyword1" => array( "title" => "退款金额", "value" => "¥" . $price . "元", "color" => "#000000" ), "keyword2" => array( "title" => "商品详情", "value" => $goods . $orderpricestr, "color" => "#000000" ), "keyword3" => array( "title" => "订单编号", "value" => $order["orderno"], "color" => "#000000" ), "remark" => array( "value" => "退款金额 ¥" . $price . (string) $refundtype . "\r\n 【" . $shopname . "】期待您再次购物！", "color" => "#000000" ) );
	...
	public function sendOrderMessage($orderid = "0", $delRefund = false, $raid = NULL) 
	{
		...
		$datas = array( array( "name" => "商城名称", "value" => $_W["shopset"]["shop"]["name"] ), array( "name" => "粉丝昵称", "value" => $member["nickname"] ), array( "name" => "订单号", "value" => $order["ordersn"] ), array( "name" => "订单编号", "value" => $order["ordersn"] ), array( "name" => "订单金额", "value" => $order["price"] ), array( "name" => "运费", "value" => $order["dispatchprice"] ), array( "name" => "商品详情", "value" => $goods ), array( "name" => "快递公司", "value" => $order["expresscom"] ), array( "name" => "快递单号", "value" => $order["expresssn"] ), array( "name" => "购买者姓名", "value" => $buyerinfo_name ), array( "name" => "购买者电话", "value" => $buyerinfo_mobile ), array( "name" => "收货地址", "value" => $addressinfo ), array( "name" => "下单时间", "value" => date("Y-m-d H:i", $order["createtime"]) ), "支付时间" => array( "name" => "支付时间", "value" => date("Y-m-d H:i", $order["paytime"]) ), array( "name" => "发货时间", "value" => date("Y-m-d H:i", $order["sendtime"]) ), array( "name" => "收货时间", "value" => date("Y-m-d H:i", $order["finishtime"]) ), array( "name" => "取消时间", "value" => date("Y-m-d H:i", $order["canceltime"]) ), array( "name" => "门店", "value" => (!empty($store) ? $store["storename"] : "") ), array( "name" => "门店地址", "value" => (!empty($store) ? $store["address"] : "") ), array( "name" => "门店联系人", "value" => (!empty($store) ? $store["realname"] . "/" . $store["mobile"] : "") ), array( "name" => "门店营业时间", "value" => (!empty($store) ? (empty($store["saletime"]) ? "全天" : $store["saletime"]) : "") ), array( "name" => "虚拟物品自动发货内容", "value" => $order["virtualsend_info"] ), array( "name" => "虚拟卡密自动发货内容", "value" => $order["virtual_str"] ), array( "name" => "自提码", "value" => $order["verifycode"] ), array( "name" => "备注信息", "value" => $order["remark"] ), array( "name" => "商品数量", "value" => $goodsnum ), array( "name" => "商品名称", "value" => $goodsname ), array( "name" => "卡路里剩余", "value" => $member["credit1"] ), array( "name" => "余额剩余", "value" => $member["credit2"] ) );
		$usernotice = unserialize($member["noticeset"]);
		if( !is_array($usernotice) ) 
		{
			$usernotice = array( );
	...
	public function sendMemberPointChange($openid, $pointchange = 0, $changetype = 0, $from = 0) 
	{
		...
		$credittext = (empty($_W["shopset"]["trade"]["credittext"]) ? "卡路里" : $_W["shopset"]["trade"]["credittext"]);
		$pointtext = "";
		$pointcolor = "";
		if( $changetype == 0 ) 
		{
		...
		$datas = array( array( "name" => "商城名称", "value" => $_W["shopset"]["shop"]["name"] ), array( "name" => "粉丝昵称", "value" => $member["nickname"] ), array( "name" => "卡路里变动", "value" => $pointtext ), array( "name" => "赠送时间", "value" => date("Y-m-d H:i", time()) ), array( "name" => "卡路里余额", "value" => (double) $member["credit1"] . $credittext ) );
		$remark = "\n[商城名称]感谢您的支持，如有疑问请联系在线客服。";
		$text = "亲爱的[粉丝昵称]， 您的" . $credittext . "发生变动，具体内容如下：\n\n卡路里变动：[卡路里变动]\n变动时间：[赠送时间]\n充值方式：" . $fromstr . "\n当前卡路里余额：[卡路里余额] \n" . $remark;
		$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，您的" . $credittext . "发生变动，具体如下:", "color" => "#ff0000" ), "keyword1" => array( "title" => "获得时间", "value" => date("Y-m-d H:i", time()), "color" => "#000000" ), "keyword2" => array( "title" => "获得卡路里", "value" => $pointtext, "color" => $pointcolor ), "keyword3" => array( "title" => "获得原因", "value" => $fromstr, "color" => "#000000" ), "keyword4" => array( "title" => "当前卡路里", "value" => (double) $member["credit1"] . $credittext, "color" => "#ff0000" ), "remark" => array( "value" => "\n" . $_W["shopset"]["shop"]["name"] . "感谢您的支持，如有疑问请联系在线客服。", "color" => "#000000" ) );
	...
	public function sendMemberLogMessage($log_id = "", $channel = 0, $isback = false) 
	{
		...
		$datas[] = array( "name" => "卡路里余额", "value" => $member["credit1"] . $credittext );
		...
		{
			if( $log_info["type"] == 1 && $log_info["status"] == 1 ) 
			{
	...
	public function sendNotice(array $params) 
	{
		...
		else 
		{
			...
			$notice_redis = m("common")->getSysset("notice_redis");
			if( !empty($notice_redis["notice_redis"]) && is_error($ret) ) 
			{
				return $ret;
			}
		}
